...
# Query the TV Episode list
cursor.execute("""
    SELECT DISTINCT
        SUBSTR(ProviderIds, INSTR(ProviderIds, 'Imdb=') + LENGTH('Imdb='),
            CASE
                WHEN INSTR(SUBSTR(ProviderIds, INSTR(ProviderIds, 'Imdb=') + LENGTH('Imdb=')), '|') > 0
                THEN INSTR(SUBSTR(ProviderIds, INSTR(ProviderIds, 'Imdb=') + LENGTH('Imdb=')), '|') - 1
